...
# ============================================================================
# SIDEBAR - Streamlined Control Panel
# ============================================================================
...
# === UNIVERSE SECTION ===
st.sidebar.markdown('<p class="section-title">📈 Watchlist</p>', unsafe_allow_html=True)
...
with preset_cols[1]:
    if st.button("TECH", use_container_width=True, key="preset_tech"):
        st.session_state["_preset_tickers"] = "AAPL, MSFT, NVDA, AMD, CRM, ADBE"
with preset_cols[2]:
    if st.button("FANG", use_container_width=True, key="preset_fang"):
        st.session_state["_preset_tickers"] = "META, AMZN, NFLX, GOOGL"
with preset_cols[3]:
    if st.button("TOP3", use_container_width=True, key="preset_top3"):
        st.session_state["_preset_tickers"] = "AAPL, MSFT, AMZN"
...
if "_preset_tickers" in st.session_state and st.session_state["_preset_tickers"]:
    ...
    tickers, _ = validate_tickers(watchlist_text)
    st.session_state["_preset_tickers"] = None

st.sidebar.markdown("---")

# ============================================================================
# PREDICTION SETTINGS - Only essential controls visible
# ============================================================================
st.sidebar.markdown('<p class="section-title">🎯 Prediction</p>', unsafe_allow_html=True)

# Horizon selector - the main control users need
...
# Trading Mode - simplified to single selector
st.sidebar.markdown('<p class="section-title">⚙️ Mode</p>', unsafe_allow_html=True)
...
if HAS_PRODUCTION_PREDICTOR and TRADING_MODES:
    selected_trading_mode = st.sidebar.selectbox(
        ...
        key="trading_mode",
        help="Balanced: Best risk/reward | Conservative: Capital preservation | Aggressive: Max returns"
    )
    use_adaptive_model = True
    
    # Show brief mode info
    mode_descriptions = {
        "balanced": "Sharpe 1.10 • 83% positive periods",
        "conservative": "Sharpe 0.68 • Lower risk",
        "aggressive": "Sharpe 1.17 • Higher returns",
    }
    st.sidebar.caption(f"📊 {mode_descriptions[selected_trading_mode]}")
else:
    use_adaptive_model = False
    selected_trading_mode = None

# ============================================================================
# DEFAULT VALUES - Set automatically (no sliders needed)
# ============================================================================

# Best performing defaults - XGBoost with auto-optimize
model_type = "xgb"  # Best performer
auto_optimize = True  # Always auto-optimize
period = "5y"  # 5 years training data

# Strategy preset defaults
strategy_preset_name = "Default (Balanced)"
strategy_preset: TradingStrategyPreset = TRADING_STRATEGY_PRESETS.get(strategy_preset_name, list(TRADING_STRATEGY_PRESETS.values())[0])

# Friction and options presets from strategy
...
# Fixed defaults (previously exposed as sliders)
signal_threshold_pct = 0.0025  # 0.25%
max_tickers = 10
min_move = 1.0
min_iv = 0.20
max_iv = 0.80
...
# Signal filter default
signal_filter_key = "baseline"
selected_filter = TRADING_STRATEGIES.get(signal_filter_key, TRADING_STRATEGIES["baseline"])

# ============================================================================
# ADVANCED OPTIONS (Collapsed - for power users)
# ============================================================================
with st.sidebar.expander("⚙️ Advanced Settings", expanded=False):
    st.caption("Override defaults for advanced use cases")
    ...
    model_override = st.selectbox(
        ...
        ["xgb", "rf", "ensemble"],
        index=0,
        format_func=lambda x: {"xgb": "XGBoost ✓", "rf": "Random Forest", "ensemble": "Ensemble (RF+XGB)"}[x],
        key="param_model"
    )
    if model_override:
    ...
    period = st.selectbox(
        ...
        ["2y", "5y", "10y"],
        index=1,
        format_func=lambda x: {"2y": "2 Years", "5y": "5 Years ✓", "10y": "10 Years"}[x],
        key="param_period"
    )
    ...
    signal_filter_key = st.selectbox(
        "Filter Mode",
    ...
    trade_mode = st.selectbox(
        "Trade Mode", 
        ["Stocks", "Options if suggested", "Options"], 
        index=1, 
        key="trade_mode_select"
    )

with st.sidebar.expander("🔬 Experimental", expanded=False):
    st.caption("Beta features - use with caution")
    
    use_elasticnet_select = st.checkbox("Elastic Net Feature Selection", value=False)
    if use_elasticnet_select:
        os.environ["USE_ELASTICNET_SELECT"] = "1"
        os.environ["ELASTICNET_L1_RATIO"] = "0.5"
        os.environ["ELASTICNET_CV_FOLDS"] = "5"
        os.environ["ELASTICNET_MINFEATURES"] = "12"
    else:
        os.environ["USE_ELASTICNET_SELECT"] = "0"
    
    run_gaf = st.checkbox("GAF-CNN Analysis", value=False, help="Image-based pattern recognition")
    run_mc = st.checkbox("Monte Carlo Simulation", value=False, help="Probabilistic price paths")

# === UTILITIES (Collapsed) ===
with st.sidebar.expander("� Utilities", expanded=False):
    st.caption("Cache & Data Management")
    ...
    with col1:
        if st.button("Predictions", use_container_width=True, key="clr_pred"):
            clear_prediction_cache()
            st.success("✓")
    with col2:
        if st.button("Prices", use_container_width=True, key="clr_price"):
            clear_price_cache()
            st.success("✓")

# === OPTIONAL FEATURES (only show if available) ===
if HAS_LIVE_UPDATES:
...
# Sidebar footer - compact status
st.sidebar.markdown("---")
st.sidebar.caption(f"🎯 {len(tickers)} tickers • {horizon_label} forecast • XGBoost")

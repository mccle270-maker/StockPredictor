...
def get_heston_params_for_ticker(ticker: str) -> HestonParams | None:
    params_by_ticker = {"AAPL": HestonParams(v0=0.04, theta=0.04, kappa=1.5, sigma=0.3, rho=-0.6), "NVDA": HestonParams(v0=0.06, theta=0.05, kappa=1.2, sigma=0.5, rho=-0.7)}
    return params_by_ticker.get(ticker.upper())

def price_atm_call_for_ticker(ticker: str, expiry: pd.Timestamp | str, spot: float, atm_iv: float | None, model: PricingModel = PricingModel.BLACK_SCHOLES, risk_free: float = 0.05, div_yield: float = 0.0) -> float | None:
    try:
        ...
        opt_spec = OptionSpec(spot=float(spot), strike=float(spot), maturity_date=expiry_date, valuation_date=val_date, rate=float(risk_free), div_yield=float(div_yield), vol=vol, is_call=True)
        heston_params = None
...
FUNDAMENTAL_COLUMNS = ["fund_pe_trailing","fund_pb","fund_market_cap"]
...
FEATURE_COLUMNS = [
"ret_1d","ret_5d","ret_20d","vol_20d","sma_ratio_10_50","rsi_14","price_to_ma50","bb_width_20",
"volume_price_corr","volume_trend","vol_ma_20","vol_spike_20","vol_rollmean_20","vol_rollstd_20",
"high_low_ratio","daily_range","close_position","hl_range","day_of_week","month","is_month_end",
"fund_pe_trailing","fund_pb","fund_market_cap","macd","macd_signal","macd_hist","mfi_14",
"ret_3d","cum_ret_3d","cum_ret_5d","ret_zscore_20d","atr_14","vol_10d","vol_60d","vol_ratio_10_60","vol_regime_high","range_atr_ratio",
"volume_zscore","dollar_volume","dollar_volume_20d_avg","ret_vol_interaction","rel_strength_1d","rel_strength_3d","rel_momentum_5d",
"rsi_change_1d","rsi_change_3d","rsi_overbought","rsi_oversold","bb_upper","bb_lower","bb_mid","bb_pct_b","ma_20","price_minus_20dma","ma_20_slope",
# NEW features (requested)
"gap_ret_1d","intraday_ret_1d","body_to_range","upper_wick_to_range","lower_wick_to_range",
"adx_14","beta_60_spx","corr_20_spx","corr_60_spx","vol_20d_std","dist_from_high_20","dist_from_low_20","up_days_5","down_days_5"
]
...
def add_price_features(hist: pd.DataFrame) -> pd.DataFrame:
    ...
    hist["cum_ret_3d"] = (1 + ret_1d_raw).rolling(3).apply(lambda x: x.prod() - 1, raw=True).shift(1)
    hist["cum_ret_5d"] = (1 + ret_1d_raw).rolling(5).apply(lambda x: x.prod() - 1, raw=True).shift(1)
    ...
    hist["bb_pct_b"] = ((close - bb_lower) / (bb_upper - bb_lower + 1e-9)).shift(1)
    ...
    hist["rsi_14"] = hist["rsi_14"].shift(1)
    ...
    # NEW: gap + candle features (all lagged)
    ...
    # NEW: ADX(14) (Wilder-style, lagged)
    up_move = high.diff()
    down_move = -low.diff()
    ...
    # NEW: distance to recent high/low + streak counts (lagged)
    ...
    # NEW: rolling beta/corr to SPX (lagged, uses same SPX returns as above if available)
    try:
        if "spx_ret_1d" not in locals():
            spx_raw2 = yf.download("^GSPC", start=hist.index[0], end=hist.index[-1], progress=False)
            if isinstance(spx_raw2.columns, pd.MultiIndex):
                spx_raw2.columns = spx_raw2.columns.get_level_values(0)
            spx_ret_1d = spx_raw2["Close"].pct_change().reindex(hist.index, method="ffill").fillna(0)
        stock_r = ret_1d_raw
...
def make_model(model_type: str = "rf", random_state: int = 42, task: str = "reg"):
    if task == "clf":
        if model_type == "xgb":
            return XGBClassifier(n_estimators=300, learning_rate=0.05, max_depth=4, random_state=random_state, tree_method="hist", verbosity=0, subsample=0.8, colsample_bytree=0.7, min_child_weight=5, reg_lambda=1.0)
        return RandomForestClassifier(n_estimators=300, max_depth=6, min_samples_leaf=50, random_state=random_state, n_jobs=-1)
    if model_type == "linreg":
        return LinearRegression()
    if model_type == "gbrt":
        return GradientBoostingRegressor(n_estimators=300, learning_rate=0.05, max_depth=4, random_state=random_state)
    if model_type == "xgb":
        return XGBRegressor(n_estimators=300, learning_rate=0.05, max_depth=4, random_state=random_state, tree_method="hist", verbosity=0, subsample=0.8, colsample_bytree=0.7, min_child_weight=5, reg_lambda=1.0)
    return RandomForestRegressor(n_estimators=300, max_depth=8, min_samples_leaf=50, random_state=random_state, n_jobs=-1)
...
def select_features_elasticnet_timeseries(X: np.ndarray, y: np.ndarray, feature_names: list[str], n_splits: int = 5, l1_ratio: float = 0.5, min_features: int = 8, random_state: int = 42) -> tuple[np.ndarray, list[str], np.ndarray]:
    ...
    pipe = Pipeline(steps=[("scaler", StandardScaler(with_mean=True, with_std=True)),
                          ("enet", ElasticNetCV(l1_ratio=float(l1_ratio), alphas=None, cv=tscv, n_jobs=-1, random_state=random_state, max_iter=5000))])
    pipe.fit(X, y)
...
def get_fundamental_features(ticker: str) -> dict:
    feats = {"fund_pe_trailing": np.nan, "fund_pb": np.nan, "fund_market_cap": np.nan}
    try:
...
def build_features_and_target(ticker="^GSPC", period="5y", horizon=1, use_vol_scaled_target: bool = False):
    ...
    for per in periods_to_try:
        try:
            ...
            hist[f"target_ret_{horizon}d_ahead"] = raw_target / (hist["vol_20d"] + 1e-9) if use_vol_scaled_target else raw_target
...
def build_features_and_direction_target(ticker="^GSPC", period="5y", horizon=1):
    X, y_reg, last_feats, last_close, last_vol_20d = build_features_and_target(ticker=ticker, period=period, horizon=horizon, use_vol_scaled_target=False)
...
def train_model(X, y, model_type="rf", test_size=0.2, random_state=42, task="reg"):
    ...
    return model, accuracy_score(y_test, y_pred), None

def predict_next_for_ticker(ticker="^GSPC", period="5y", model_type="rf", horizon=1, use_vol_scaled_target: bool = False, auto_optimize: bool = True):
    X, y, x_last, last_close, last_vol_20d = build_features_and_target(ticker, period=period, horizon=horizon, use_vol_scaled_target=use_vol_scaled_target)
    feat_cols = FEATURE_COLUMNS + MACRO_COLUMNS
    n = len(X)
    if USE_ELASTICNET_SELECT:
        try:
            ...
            X_en_train_sel, en_selected_names, en_mask = select_features_elasticnet_timeseries(X=X_en_train, y=y_en_train, feature_names=list(feat_cols), n_splits=ELASTICNET_CV_FOLDS, l1_ratio=ELASTICNET_L1_RATIO, min_features=10)
            X = X[:, en_mask]
    ...
    return {"ticker": ticker, "model_type": model_type, "horizon": horizon, "last_close": last_close, "vol_20d": last_vol_20d, "pe_ratio": pe_ratio, "pred_next_ret": pred_ret, "pred_next_price": pred_price, "prob_up": prob_up, "prob_down": prob_down, "prob_up_gaf": prob_up_gaf, "num_features": len(feat_cols), "top_features": top_features_str, "elasticnet_enabled": bool(USE_ELASTICNET_SELECT), "elasticnet_l1_ratio": float(ELASTICNET_L1_RATIO), "elasticnet_cv_folds": int(ELASTICNET_CV_FOLDS), "elasticnet_selected_n": int(len(feat_cols)) if USE_ELASTICNET_SELECT else None}
...
def track_predictions(ticker, period="1y", model_type="rf", horizon=1):
    try:
        ...
        if USE_ELASTICNET_SELECT:
            try:
                X_train_sel, selected_names, selected_mask = select_features_elasticnet_timeseries(X=X_train, y=y_train, feature_names=list(feat_cols), n_splits=ELASTICNET_CV_FOLDS, l1_ratio=ELASTICNET_L1_RATIO, min_features=10)
                X_train = X_train_sel
        ...
        y_pred = model.predict(X_test)
        results = pd.DataFrame({"date": test_df.index, "actual_close": hist.loc[test_df.index, "Close"], "predicted_return": y_pred, "actual_return": y_test, "pred_direction": np.sign(y_pred), "actual_direction": np.sign(y_test), "correct_direction": np.sign(y_pred) == np.sign(y_test)})
...
def backtest_one_ticker(ticker="AAPL", period="10y", test_years=1, threshold=0.002, model_type="rf", horizon=1):
    ...
    if USE_ELASTICNET_SELECT:
        try:
            X_train_sel, selected_feats, selected_mask = select_features_elasticnet_timeseries(X=X_train, y=y_train, feature_names=list(feat_cols), n_splits=ELASTICNET_CV_FOLDS, l1_ratio=ELASTICNET_L1_RATIO, min_features=10)
            X_train = X_train_sel
    ...
    return {"ticker": ticker, "model_type": model_type, "horizon": horizon, "test_days": len(pnl), "total_return": cum_ret, "hit_rate": hit_rate, "sharpe": sharpe, "elasticnet_enabled": bool(USE_ELASTICNET_SELECT), "elasticnet_selected_features": selected_feats, "elasticnet_selected_n": len(selected_feats) if selected_feats is not None else None}

def backtest_one_ticker_auto_optimized(ticker="AAPL", period="10y", test_years=2, threshold=0.002, model_type="rf", horizon=5, importance_threshold=0.001):
    hist = get_price_history(ticker, period=period, interval="1d")
    hist = add_price_features(hist)
    macro_df = get_macro_df(symbol="^GSPC", period=period)
    hist = hist.join(macro_df, how="left")
    ...
    std_daily = pnl.std(ddof=1)
    sharpe = np.sqrt(252) * avg_daily / std_daily if std_daily > 0 else 0.0
    return {"ticker": ticker, "model_type": model_type, "horizon": horizon, "num_features_original": len(FEATURE_COLUMNS) + len(MACRO_COLUMNS), "num_features_used": len(important_features), "features_dropped": dropped_count, "test_days": len(pnl), "total_return": cum_ret, "hit_rate": hit_rate, "sharpe": sharpe, "elasticnet_enabled": bool(USE_ELASTICNET_SELECT), "elasticnet_selected_n": int(selected_mask.sum()) if selected_mask is not None else None}

def backtest_compare_one_ticker(ticker="AAPL", period="10y", test_years=1, threshold=0.002, horizon=1, auto_optimize=True):
    if auto_optimize:
        rf_res = backtest_one_ticker_auto_optimized(ticker=ticker, period=period, test_years=test_years, threshold=threshold, model_type="rf", horizon=horizon)
        gbrt_res = backtest_one_ticker_auto_optimized(ticker=ticker, period=period, test_years=test_years, threshold=threshold, model_type="gbrt", horizon=horizon)
        xgb_res = backtest_one_ticker_auto_optimized(ticker=ticker, period=period, test_years=test_years, threshold=threshold, model_type="xgb", horizon=horizon)
    else:
        rf_res = backtest_one_ticker(ticker=ticker, period=period, test_years=test_years, threshold=threshold, model_type="rf", horizon=horizon)
        gbrt_res = backtest_one_ticker(ticker=ticker, period=period, test_years=test_years, threshold=threshold, model_type="gbrt", horizon=horizon)
        xgb_res = backtest_one_ticker(ticker=ticker, period=period, test_years=test_years, threshold=threshold, model_type="xgb", horizon=horizon)
    return {"rf": rf_res, "gbrt": gbrt_res, "xgb": xgb_res}

def walk_forward_backtest(ticker="AAPL", period="10y", horizon=1, model_type="rf", train_years=4, test_years=1, threshold=0.002, cost_per_trade=0.0005, step_days: int | None = None):
    hist = get_price_history(ticker, period=period, interval="1d")
    if hist is None or hist.empty:
        return []
    hist = add_price_features(hist)
    ...
    while True:
        ...
        if USE_ELASTICNET_SELECT:
            try:
                X_train_sel, fold_feats, fold_mask = select_features_elasticnet_timeseries(X=X_train, y=y_train, feature_names=list(feat_cols), n_splits=ELASTICNET_CV_FOLDS, l1_ratio=ELASTICNET_L1_RATIO, min_features=10)
                X_train = X_train_sel
        ...
        std_daily = pnl.std(ddof=1)
        sharpe = np.sqrt(252) * avg_daily / std_daily if std_daily > 0 else 0.0
        num_trades = int(np.count_nonzero(np.diff(np.concatenate([[0], positions])) != 0))
        fold_metrics.append({"train_start": train_df.index[0], "train_end": train_df.index[-1], "test_start": test_df.index[0], "test_end": test_df.index[-1], "test_days": len(pnl), "hit_rate": hit_rate, "sharpe": sharpe, "num_trades": num_trades})
        start += step_days
    return fold_metrics

def analyze_feature_significance(ticker="^GSPC", period="5y", horizon=1, use_vol_scaled_target: bool = False, alpha: float = 0.05):
    X, y, _, _, _ = build_features_and_target(ticker=ticker, period=period, horizon=horizon, use_vol_scaled_target=use_vol_scaled_target)
    feat_cols = FEATURE_COLUMNS + MACRO_COLUMNS
    ...
    for name in ordered_names:
        if name in ols_model.params.index:
            ...
            rows.append({"feature": name, "coef": float(ols_model.params[name]), "p_value": p_val, "significant": bool(p_val < alpha)})
...
def predict_up_gaf_cnn(ticker: str, window: int = 30, image_size: int = 30, period: str = "3y") -> float | None:
    if gaf_cnn is None:
        return None
    hist = get_price_history(ticker, period=period, interval="1d")
...
def tune_xgb_hyperparams(X, y, random_state=42):
    ...
    param_distributions = {"learning_rate":[0.01,0.03,0.05,0.1],"n_estimators":[200,400,600],"max_depth":[3,4,5],"min_child_weight":[1,3,5],"subsample":[0.7,0.9,1.0],"colsample_bytree":[0.7,0.9,1.0],"reg_lambda":[0.0,1.0,5.0]}
    search = RandomizedSearchCV(estimator=base_model, param_distributions=param_distributions, n_iter=20, scoring="neg_mean_squared_error", cv=tscv, random_state=random_state, n_jobs=-1, verbose=1)
    search.fit(X, y)

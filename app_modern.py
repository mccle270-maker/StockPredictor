...
WATCHLISTS_PATH = BASE_DIR / ".config" / "watchlists.json"
...
def _save_watchlists_file(data: dict) -> bool:
    """Save watchlists to JSON file."""
    try:
        WATCHLISTS_PATH.parent.mkdir(parents=True, exist_ok=True)
        WATCHLISTS_PATH.write_text(json.dumps(data, indent=2))
        return True
    except Exception:
        return False

def save_watchlist(name: str, tickers: list) -> bool:
    """
    Save a watchlist by name.
    
    Args:
        name: Watchlist name (e.g., "My Tech Stocks")
    ...
    data = _load_watchlists_file()
    data[name] = {
        "tickers": tickers,
        "updated": datetime.now().isoformat(),
    }
    return _save_watchlists_file(data)

def load_watchlist(name: str) -> list:
    """
    ...
    Returns:
        List of ticker symbols, or empty list if not found
    """
    data = _load_watchlists_file()
    watchlist = data.get(name, {})
    return watchlist.get("tickers", [])

def get_saved_watchlist_names() -> list:
    """Get list of all saved watchlist names."""
    data = _load_watchlists_file()
...
def render_floating_action_button():
    """Render a floating action button for quick prediction run."""
    c = get_colors()
    ...
    <style>
        ...
        .fab-button {{
            background: linear-gradient(135deg, {c['accent_blue']}, {c['accent_purple']});
            border: none;
            border-radius: 28px;
            padding: 16px 24px;
            color: white;
        ...
        .fab-button:hover {{
            ...
            box-shadow: 0 6px 28px rgba(56, 139, 253, 0.5);
        }}
        .fab-button:active {{
    ...
    <div class="fab-container">
        <div class="fab-button" onclick="document.querySelector('[data-testid=\\'stButton\\'] button').click()">
            🚀 Run Predictions
            <span class="fab-shortcut">⌘↵</span>
        </div>
    </div>
    """
...
def render_keyboard_hints():
    """Show keyboard shortcuts in bottom left."""
    c = get_colors()
    ...
    <style>
        ...
        .kbd-hints summary::-webkit-details-marker {{
            display: none;
        }}
        .kbd-hints .shortcuts {{
            padding-top: 8px;
...
def get_market_status_badge():
    ...
    # Check if market is open (simplified - Mon-Fri 9:30 AM - 4:00 PM ET)
    from datetime import datetime
    import pytz
    ...
    try:
        ...
        if is_market_hours:
            return f'<span style="color: {c["accent_green"]}; margin-right: 16px; font-size: 0.8rem;">🟢 MARKET OPEN</span>'
        else:
            return f'<span style="color: {c["accent_yellow"]}; margin-right: 16px; font-size: 0.8rem;">🟡 AFTER HOURS</span>'
    except:
        return f'<span style="color: {c["text_muted"]}; margin-right: 16px; font-size: 0.8rem;">⚪ MARKET STATUS</span>'


def render_price_ticker_strip(tickers_data: dict):
    """
    Render a Bloomberg-style scrolling price ticker.
    
    Args:
        tickers_data: Dict of {symbol: {"price": float, "change": float, "change_pct": float}}
    """
    c = get_colors()
    ...
    market_status = get_market_status_badge()
    ...
    for symbol, data in tickers_data.items():
        ...
        change_color = c['accent_green'] if change >= 0 else c['accent_red']
...
def get_ticker_strip_data(symbols: list) -> dict:
    """Get price data for ticker strip display."""
    data = {}
    
    for symbol in symbols[:8]:  # Limit to 8 for performance
        try:
            # Try to get from cached predictions first
            pred_df = st.session_state.get("pred_df")
            if pred_df is not None and "ticker" in pred_df.columns:
                row = pred_df[pred_df["ticker"] == symbol]
                if not row.empty:
                    last_close = row.iloc[0].get("last_close", 0) if "last_close" in row.columns else 0
                    pred_ret = row.iloc[0].get("pred_next_ret", 0) if "pred_next_ret" in row.columns else 0
                    pred_ret = pred_ret if pred_ret else 0
                    data[symbol] = {
                        "price": float(last_close) if last_close else 0,
                        "change": float(last_close * pred_ret) if last_close else 0,
                        "change_pct": float(pred_ret * 100),
                    }
                    continue
            
            # Fallback: use placeholder data for demo
            # In production, you'd call get_live_price here
            import random
            base_prices = {
                "SPY": 478.50, "QQQ": 412.30, "AAPL": 185.20, "NVDA": 495.80,
                "MSFT": 378.40, "GOOGL": 142.15, "AMZN": 153.60, "TSLA": 248.90
            }
...
def render_empty_state(
    ...
    """Render a professional empty state with optional action button."""
    c = get_colors()
    ...
    return f"""
    <div style="
        background: linear-gradient(135deg, {c['bg_card']} 0%, {c['bg_secondary']} 100%);
...
def render_error_state(title: str, description: str, suggestion: str = None):
    """Render an error state with helpful suggestions."""
    c = get_colors()
    ...
    if suggestion:
        ...
        <div style="
            ...
            text-align: left;
        ">
            <span style="color: {c['accent_yellow']};">💡 Tip: </span>
            <span style="color: {c['text_secondary']};">{suggestion}</span>
        </div>
        """
    
    return f"""
    <div style="
        background: {c['accent_red']}10;
        border: 1px solid {c['accent_red']}40;
        border-radius: 12px;
        padding: 2rem;
        text-align: center;
        margin: 1rem 0;
    ">
        ...
        <h4 style="color: {c['accent_red']}; margin: 0 0 0.5rem 0;">{title}</h4>
        <p style="color: {c['text_secondary']}; margin: 0;">{description}</p>
        {suggestion_html}
    </div>
    """
    ...
    try:
        ...
        try:
            ...
            if df is not None and len(df) >= 60:
                ...
                # Trend structure
                trend_detector = TrendStructureDetector(lookback=60)
...
# ============================================================================
# CACHED DATABASE CALLS
# ============================================================================

@st.cache_data(ttl=60, show_spinner=False)
def _cached_recent_queries(limit: int = 5, query_type: str = "prediction"):
    """Cached version of get_recent_queries. TTL: 60 seconds."""
    if HAS_DATABASE:
        return get_recent_queries(limit=limit, query_type=query_type)
    return []

@st.cache_data(ttl=120, show_spinner=False)
def _cached_database_stats():
    """Cached version of get_database_stats. TTL: 2 minutes."""
    if HAS_DATABASE:
        return get_database_stats()
    return {}

@st.cache_data(ttl=300, show_spinner=False)
def _cached_accuracy_stats(ticker: str = None, model_type: str = None, days: int = 30):
    """Cached version of get_accuracy_stats. TTL: 5 minutes."""
    if HAS_DATABASE:
        return get_accuracy_stats(ticker=ticker, model_type=model_type, days=days)
    return {}
    ...
    if pred_df is None or pred_df.empty:
        st.markdown(
            render_empty_state(
                ...
                description="Enter tickers in the sidebar and click Run Predictions to generate ML-powered trading signals.",
                icon="🚀",
...
# Custom input styling to override Streamlit defaults
st.markdown("""
<style>
    /* ============================================
       CUSTOM INPUT STYLING - Override Streamlit defaults
       ============================================ */
    ...
    /* Tabs styling (if any native tabs remain) */
    .stTabs [data-baseweb="tab-list"] {
        gap: 0 !important;
        background: transparent !important;
    }
...
# === UNIVERSE SECTION ===
st.sidebar.markdown(section_header("Universe", subtitle="Select tickers to analyze"), unsafe_allow_html=True)

# Initialize session state for ticker input if not exists
if "watchlist_tickers" not in st.session_state:
    st.session_state["watchlist_tickers"] = "AAPL, NVDA, MSFT"
...
if watchlist_text != st.session_state.get("watchlist_tickers"):
    st.session_state["watchlist_tickers"] = watchlist_text
...
with preset_cols[0]:
    if st.button("MAG7", use_container_width=True, key="preset_mag7"):
        st.session_state["watchlist_tickers"] = "AAPL, NVDA, MSFT, GOOGL, AMZN, META, TSLA"
        st.rerun()
with preset_cols[1]:
    if st.button("TECH", use_container_width=True, key="preset_tech"):
        st.session_state["watchlist_tickers"] = "AAPL, MSFT, NVDA, AMD, CRM, ADBE, INTC"
        st.rerun()
with preset_cols[2]:
    if st.button("FANG", use_container_width=True, key="preset_fang"):
        st.session_state["watchlist_tickers"] = "META, AMZN, NFLX, GOOGL"
        st.rerun()

# Saved Watchlists
saved_watchlists = get_saved_watchlist_names()
if saved_watchlists:
    with st.sidebar.expander("📂 Saved Watchlists", expanded=False):
        selected_wl = st.selectbox(
            "Load watchlist",
            options=[""] + saved_watchlists,
            key="select_saved_watchlist",
            label_visibility="collapsed"
        )
        if selected_wl:
            wl_cols = st.columns([2, 1])
            with wl_cols[0]:
                if st.button("📥 Load", key="load_watchlist", use_container_width=True):
                    loaded_tickers = load_watchlist(selected_wl)
                    st.session_state["watchlist_tickers"] = ", ".join(loaded_tickers)
                    st.rerun()
            with wl_cols[1]:
                if st.button("🗑️", key="delete_watchlist", help="Delete this watchlist"):
                    delete_watchlist(selected_wl)
                    st.rerun()

# Save current watchlist
with st.sidebar.expander("💾 Save Current", expanded=False):
    new_wl_name = st.text_input("Watchlist name", key="new_watchlist_name", placeholder="My Watchlist")
    if st.button("💾 Save Watchlist", key="save_watchlist_btn", use_container_width=True):
        if new_wl_name and tickers:
            if save_watchlist(new_wl_name, tickers):
                show_toast(f"✅ Saved '{new_wl_name}' with {len(tickers)} tickers", "success")
            else:
                show_alert("Failed to save watchlist", "error")
        elif not new_wl_name:
            show_alert("Please enter a watchlist name", "warning")
        else:
            show_alert("No valid tickers to save", "warning")

# ============================================================================
# SIMPLIFIED SIDEBAR - Master Strategy Preset
# ============================================================================
st.sidebar.markdown(section_header("🎯 Trading Strategy", subtitle="Choose your trading approach"), unsafe_allow_html=True)
...
st.sidebar.markdown(section_header("📊 Signal Filter", subtitle="Quality thresholds for signals"), unsafe_allow_html=True)
...
with st.sidebar.expander("⚙️ Advanced Options", expanded=False):
    ...
    all_models = ["xgb", "rf", "xgb_lstm", "ensemble"]
...
# === CACHE MANAGEMENT ===
st.sidebar.markdown(section_header("Cache Management", subtitle="Clear cached data"), unsafe_allow_html=True)
with st.sidebar.expander("🗑️ Clear Caches", expanded=False):
    ...
    with cache_cols[0]:
        if st.button("Clear Predictions", use_container_width=True):
            clear_prediction_cache()
            show_toast("✓ Predictions cleared", "success")
    with cache_cols[1]:
        if st.button("Clear Prices", use_container_width=True):
            clear_price_cache()
            show_toast("✓ Prices cleared", "success")
    cache_cols2 = st.columns(2)
    with cache_cols2[0]:
        if st.button("Clear Options", use_container_width=True):
            clear_options_cache()
            show_toast("✓ Options cleared", "success")
    with cache_cols2[1]:
        if st.button("Clear Backtests", use_container_width=True):
            clear_backtest_cache()
            show_toast("✓ Backtests cleared", "success")
    if st.button("🔄 Clear All Caches", use_container_width=True, type="primary"):
        clear_all_caches()
        show_toast("✓ All caches cleared!", "success")

# === OPTIONAL FEATURES ===
if HAS_LIVE_UPDATES:
    st.sidebar.markdown(section_header("Live Updates", subtitle="Real-time market data"), unsafe_allow_html=True)
    init_live_state()
...
if HAS_DATABASE:
    st.sidebar.markdown(section_header("History", subtitle="Recent prediction queries"), unsafe_allow_html=True)
    with st.sidebar.expander("📚 Query History", expanded=False):
        recent_queries = _cached_recent_queries(limit=5, query_type="prediction")
        if recent_queries:
...
# ============================================================================
# SCROLLING PRICE TICKER STRIP (Disabled - CSS rendering issues)
# ============================================================================

# The ticker strip is temporarily disabled due to CSS rendering issues in Streamlit
# TODO: Re-enable when CSS injection is fixed
# strip_tickers = tickers if tickers else ["SPY", "QQQ", "AAPL", "NVDA", "MSFT", "GOOGL", "AMZN", "TSLA"]
# strip_data = get_ticker_strip_data(strip_tickers[:8])
# if strip_data:
#     st.markdown(render_price_ticker_strip(strip_data), unsafe_allow_html=True)
...
if active_tab == "summary":
    ...
    if pred_df is not None and not pred_df.empty:
        ...
        # === FILTERING CONTROLS ===
        st.markdown(section_header("🔍 Filter Opportunities"), unsafe_allow_html=True)
        ...
        with filter_cols[1]:
            confidence_filter = st.selectbox(
                ...
                help="High = prob_up >60% or <40% (model is sure); Low = near 50% (uncertain)"
            )
        ...
        # Confidence filter (based on distance from 50%)
        if "prob_up" in filtered_df.columns:
            if confidence_filter == "High Confidence":
                # Model is confident: prob_up > 60% (bullish) or < 40% (bearish)
                filtered_df = filtered_df[(filtered_df["prob_up"] > 0.6) | (filtered_df["prob_up"] < 0.4)]
            elif confidence_filter == "Medium Confidence":
                # Moderate confidence: between 45-55% excluded, but not extreme
                filtered_df = filtered_df[
                    ((filtered_df["prob_up"] >= 0.4) & (filtered_df["prob_up"] <= 0.45)) |
                    ((filtered_df["prob_up"] >= 0.55) & (filtered_df["prob_up"] <= 0.6))
                ]
            elif confidence_filter == "Low Confidence":
                # Model is uncertain: prob_up between 45-55% (near coin flip)
                filtered_df = filtered_df[(filtered_df["prob_up"] >= 0.45) & (filtered_df["prob_up"] <= 0.55)]
        ...
        if not has_arima:
            ...
            with col_info:
                show_alert("💡 ARIMA signals not available. Click 'Clear Cache & Refresh' to fetch fresh predictions with ARIMA analysis.", "info")
            with col_btn:
                if st.button("🔄 Clear Cache & Refresh", key="summary_clear_cache", use_container_width=True):
                    st.cache_data.clear()
        ...
        if filtered_df.empty:
            show_alert("No tickers match your filter criteria. Try adjusting the filters above.", "info")
        else:
            ...
            for row_idx in range(rows_needed):
                ...
                for col_idx in range(num_cols):
                    ...
                    # Calculate a more meaningful confidence based on P(Up) distance from 50%
                    # If P(Up) = 70%, confidence = |70-50|*2 = 40% (capped at 100%)
                    if prob_up is not None:
                        conf_pct = min(abs(prob_up - 0.5) * 2, 1.0)  # Scale 0-0.5 to 0-1
                    else:
                        conf_pct = None
                    ...
                    conf_color = tc["green"] if conf_pct and conf_pct > 0.3 else tc["yellow"] if conf_pct and conf_pct > 0.1 else tc["text"]
            ...
            with action_cols[0]:
                if st.button("📈 Go to Dashboard", key="summary_to_dash", use_container_width=True):
                    show_alert("Switch to the Dashboard tab for detailed analysis", "info")
            with action_cols[1]:
                if st.button("🔬 Run Backtest", key="summary_to_backtest", use_container_width=True):
                    show_alert("Switch to the Backtest tab to validate signals", "info")
            with action_cols[2]:
                if st.button("📊 View Portfolio", key="summary_to_portfolio", use_container_width=True):
                    show_alert("Switch to the Portfolio tab for position management", "info")
            with action_cols[3]:
                # Export summary
                if st.button("📥 Export CSV", key="summary_export", use_container_width=True):
        ...
        st.markdown(
            render_empty_state(
                title="No Predictions Yet",
                description="Run predictions from the Dashboard tab to see your quick summary here with at-a-glance cards for each ticker.",
                icon="🚀",
                action_text="Go to Dashboard →"
            ),
            unsafe_allow_html=True
        ...
        show_alert("👉 **Tip**: Go to the Dashboard tab, enter your tickers, and click 'Run Predictions' to get started!", "info")
...
# ============================================================================
    ...
    if run_btn:
        ...
        if not is_valid:
            ...
            show_alert("⚠️ Enter at least one ticker symbol in the sidebar", "warning")
        else:
            results = []
            errors = []
            ...
            for i, tk in enumerate(tickers):
                ...
                if error:
                    ...
                    errors.append((tk, f"No prediction returned for {tk}"))
                elif pred:
            ...
            if results:
                ...
                try:
                    ...
                    show_toast(f"✅ Wrote signals.json to: {SIGNALS_OUT_PATH}", "success")
                except Exception as e:
                    show_alert(f"Failed to write signals: {e}", "warning")
                ...
                if auto_run_trader:
                    if not TRADER_PATH.exists():
                        show_alert(f"Trader not found: {TRADER_PATH}", "error")
                    else:
                        # Show loading spinner
                        loading_placeholder = st.empty()
                        loading_placeholder.markdown(loading_spinner(message="Executing trades..."), unsafe_allow_html=True)
                        ...
                        if res.returncode == 0:
                            show_toast("Trade execution complete", "success")
                        else:
                            show_alert(f"Trade failed: code {res.returncode}", "error")
    ...
    if pred_df is not None and not pred_df.empty:
        ...
        st.markdown(section_header("Market Analytics"), unsafe_allow_html=True)
        ...
        st.markdown(section_header("Market Summary"), unsafe_allow_html=True)
        ...
        if has_arima_data:
            st.markdown(section_header("ARIMA Signal Summary"), unsafe_allow_html=True)
        ...
        # === SIGNALS TABLE ===
        st.markdown(section_header("Trading Signals"), unsafe_allow_html=True)
        ...
        if "vol_scaled_quantity" not in cand_df.columns:
            ...
            if "pred_next_ret" in cand_df.columns and "vol_20d" in cand_df.columns:
                ...
                # scale = edge/vol, capped at 3x (avoid division by zero)
                vol_safe = cand_df["vol_20d"].replace(0, np.nan)
                cand_df["vol_scale_factor"] = (cand_df["pred_next_ret"].abs() / vol_safe).clip(upper=3.0).fillna(1.0)
        ...
        st.markdown(section_header("P&L and Risk Summary"), unsafe_allow_html=True)
        ...
        with st.expander("P&L and Risk Summary (Backtest Metrics)", expanded=False):
            if ticker_list:
                ...
                metrics_placeholder.markdown(loading_spinner(message="Loading backtest metrics..."), unsafe_allow_html=True)
                ...
                if not pnl_df.empty:
                    ...
                    show_alert("No backtest data available for selected tickers.", "info")
            else:
                show_alert("Run predictions first to see P&L and risk metrics.", "info")
        ...
        st.markdown(section_header("Signal Analytics"), unsafe_allow_html=True)
        ...
        if pred_df is not None and not pred_df.empty and "pred_next_ret" in pred_df.columns:
            ...
            with chart_cols[1]:
                ...
                if "prob_up" in pred_df.columns:
                    ...
                    show_alert("P(Up) data not available for scatter plot", "info")
                ...
                if "vol_20d" in pred_df.columns and "atm_iv" in pred_df.columns:
                    ...
                    if not vol_df.empty:
                        ...
                        fig_vol_scatter.add_trace(go.Scatter(
                            ...
                            textposition="top center",
                            textfont=dict(size=9, color=THEME["text_primary"]),
                            customdata=vol_df["pred_next_ret"].apply(lambda x: f"{x*100:+.2f}%").tolist(),
                            hovertemplate="<b>%{text}</b><br>RV: %{x:.1f}%<br>IV: %{y:.1f}%<br>Pred: %{customdata}<extra></extra>"
                        ))
                        ...
                        show_alert("Insufficient volatility data", "info")
                else:
                    ...
                    if "prob_up" in pred_df.columns:
                        ...
                        show_alert("No probability data available", "info")
        
        st.markdown(divider(), unsafe_allow_html=True)
        
        # === OPTION STRATEGIES SECTION ===
        st.markdown(section_header("Option Strategies"), unsafe_allow_html=True)
        ...
        try:
            ...
            if strategy_df is not None and not strategy_df.empty:
                ...
                show_alert("No option strategies could be generated. Ensure predictions include required fields (pred_next_ret, atm_iv, vol_20d).", "info")
        except Exception as e:
            show_alert(f"Could not generate option strategies: {e}", "warning")
        ...
        if run_all_acc:
            if pred_df is None or pred_df.empty:
                show_alert("Run predictions first", "warning")
            else:
        ...
        st.markdown(section_header("Ticker Analysis"), unsafe_allow_html=True)
        
        if not detail_universe:
            st.markdown(
                render_empty_state(
                    title="No Tickers Available",
                    description="Run predictions first to analyze individual stocks in detail.",
                    icon="🔍"
                ),
                unsafe_allow_html=True
            )
            ...
            if matching_rows.empty:
                show_alert(f"No data for {selected}", "error")
            else:
                ...
                # === PRIMARY METRICS ROW ===
                st.markdown(section_header("Prediction Metrics"), unsafe_allow_html=True)
                ...
                st.markdown(section_header("Additional Data"), unsafe_allow_html=True)
                ...
                if has_arima:
                    st.markdown(section_header("ARIMA Signals"), unsafe_allow_html=True)
                ...
                if hist is None or hist.empty or "Close" not in hist.columns:
                    show_alert("No price data available", "info")
                else:
                ...
                with detail_cols[1]:
                    with st.expander("📈 30D Outlook", expanded=False):
                        ...
                        if "error" in lh_result:
                            show_alert(f"Unavailable: {lh_result.get('error', '')[:50]}", "info")
                        else:
                ...
                with st.expander("📉 Risk Dashboard", expanded=False):
                    ...
                    if hist_full is None or hist_full.empty or "Close" not in hist_full.columns:
                        show_alert("No history available to compute risk metrics.", "info")
                    else:
                        ...
                        if not dd_df.empty and "drawdown" in dd_df.columns:
                            ...
                            show_alert("Drawdown data unavailable", "info")
                        ...
                        with risk_cols[0]:
                            ...
                            if not sharpe_60.empty:
                                ...
                                show_alert("Sharpe data unavailable", "info")
                        ...
                        with risk_cols[1]:
                            ...
                            if not sortino_60.empty:
                                ...
                                show_alert("Sortino data unavailable", "info")
                        ...
                        if not vol_20.empty:
                            ...
                            show_alert("Volatility data unavailable", "info")
                        ...
                        if not var95.empty and not es95.empty:
                            ...
                            show_alert("VaR/ES data unavailable", "info")
                    ...
                    try:
                        ...
                        if news:
                            ...
                            if detect_big_news(news):
                                show_alert("⚠️ **Recent significant news detected!** Consider increased volatility.", "warning")
                            
                            tc = get_card_theme()
                            st.markdown(section_header("Recent Headlines"), unsafe_allow_html=True)
                            ...
                            show_alert("📰 No recent news available for this ticker. News API may not be configured.", "info")
                    except Exception as e:
                        show_alert(f"📰 News unavailable: API not configured or rate limited.", "info")
                ...
                with st.expander("📋 Signals & Output", expanded=False):
                    ...
                    if st.session_state.get("last_signals"):
                        ...
                        if sig_rows:
                            ...
                            # Show warnings
                            if weak_count > 0:
                                show_alert(f"⚠️ {weak_count} weak signal(s) included. These have |z-score| < 1.0 and may be less reliable.", "warning")
                            if skipped_count > 0:
                                show_alert(f"⏭️ {skipped_count} signal(s) skipped due to trade limits. Only top-ranked signals per ticker are allowed.", "info")
                            if regime_blocked_count > 0:
                                show_alert(f"🚫 {regime_blocked_count} signal(s) blocked by market regime filter. Trade direction not allowed in current regime.", "error")
                    else:
                        show_alert("No signals generated yet. Run predictions first.", "info")
                    
                    if st.session_state.get("last_trader_rc") is not None:
                        rc = st.session_state.last_trader_rc
                        if rc == 0:
                            show_toast("✅ Trade execution completed successfully", "success")
                        else:
                            show_alert(f"❌ Trade execution failed (code {rc})", "error")
        ...
        st.markdown(
            render_empty_state(
                ...
                description="Enter your tickers in the sidebar and click RUN PREDICTIONS to get started with ML-powered analysis.",
                icon="📈",
                action_text="Run Predictions"
            ),
            unsafe_allow_html=True
        )
    ...
    with st.expander("📉 Z-Score Weak Signals Log", expanded=False):
        ...
        **What is this?**  
        ...
        try:
            ...
            show_alert(f"Current threshold: |z-score| ≥ {threshold}", "info")
            ...
            if log_path.exists():
                ...
                if weak_logs:
                    ...
                    # Use styled_table for modern look
                    st.markdown(styled_table(
                        weak_df[display_cols].tail(50),
                        highlight_rules={"z_score": "number", "pred": "percent"}
                    ), unsafe_allow_html=True)
                    ...
                    if st.button("🗑️ Clear Weak Signals Log", key="clear_weak_log"):
                        log_path.unlink()
                        show_toast("Log cleared!", "success")
                        st.rerun()
                else:
                    show_toast("✅ No weak signals logged yet!", "success")
            else:
                show_alert("No weak signals log file found. Signals will be logged as predictions are made.", "info")
        except Exception as e:
            show_alert(f"Could not load weak signals log: {e}", "warning")
        ...
        with st.expander("📚 Prediction History Explorer", expanded=False):
            ...
            try:
                ...
                if not history_df.empty:
                    ...
                    # Use styled_table for modern look
                    st.markdown(styled_table(display_df), unsafe_allow_html=True)
                    
                    # Accuracy summary (cached)
                    accuracy_stats = _cached_accuracy_stats(
                        ticker=hist_ticker if hist_ticker else None,
                        model_type=hist_model if hist_model != "All" else None,
                        days=hist_days
                    )
                    ...
                    show_alert("No prediction history found for the selected filters.", "info")
                    
            except Exception as e:
                show_alert(f"Could not load history: {e}", "warning")
...
# ============================================================================
    ...
    st.markdown(section_header("Single Stock Backtest", subtitle="Test model performance on historical data"), unsafe_allow_html=True)
    ...
    with input_cols[3]:
        bt_period = st.selectbox("Period", ["1y", "2y", "5y", "10y", "Custom"], index=2, key="bt_period")
    with input_cols[4]:
    ...
    if bt_period == "Custom":
        ...
        with date_cols[2]:
            st.caption(f"📅 Custom range: {bt_start_date} to {bt_end_date}")
    
    st.markdown(divider(), unsafe_allow_html=True)
    
    if run_backtest_btn:
        # Validate backtest inputs
        # For custom dates, skip period validation
        if bt_period != "Custom":
            is_valid, validation_errors = validate_backtest_inputs(bt_ticker, bt_period, bt_horizon, bt_model)
        else:
            is_valid, validation_errors = validate_backtest_inputs(bt_ticker, "5y", bt_horizon, bt_model)
        ...
        if not is_valid:
            ...
            # Determine effective period for data fetching
            if bt_period == "Custom":
                # Calculate approximate period from date range
                ...
                if days_diff < 365:
                    ...
                    effective_period = "10y"
            else:
                effective_period = bt_period
            ...
            if api_error:
                ...
                show_alert(f"⚠️ No data returned for {bt_ticker_clean}. The ticker may be invalid or have insufficient history.", "warning")
            else:
                results_test, accuracy = result_data
                
                # Apply custom date range filter if specified
                if bt_period == "Custom" and results_test is not None and not results_test.empty:
                    try:
                        # Convert dates to datetime for comparison
                        start_dt = pd.Timestamp(bt_start_date)
                        end_dt = pd.Timestamp(bt_end_date)
                        
                        # Ensure index is datetime
                        if not isinstance(results_test.index, pd.DatetimeIndex):
                            results_test.index = pd.to_datetime(results_test.index)
                        
                        # Filter to date range
                        results_test = results_test[(results_test.index >= start_dt) & (results_test.index <= end_dt)]
                    except Exception as date_err:
                        show_alert(f"⚠️ Could not apply date filter: {date_err}", "warning")
                
                if results_test is None or results_test.empty:
                    show_alert(f"⚠️ Insufficient historical data for backtest on {bt_ticker_clean}. Try a shorter time period or different ticker.", "warning")
                else:
                    try:
                        baseline_returns = results_test["actual_return"].dropna()
                        
                        if len(baseline_returns) < 20:
                            show_alert(f"⚠️ Only {len(baseline_returns)} data points available. Results may not be statistically significant.", "warning")
                        ...
                        st.markdown(section_header("Cumulative Performance"), unsafe_allow_html=True)
        ...
        st.markdown(
            render_empty_state(
                title="Configure Your Backtest",
                description="Select a ticker, model, and time period above, then click RUN BACKTEST to evaluate historical performance.",
                icon="🔬"
            ),
            unsafe_allow_html=True
        )
    ...
    # ========================================================================
    st.markdown(divider(), unsafe_allow_html=True)
    st.markdown(section_header("📊 Model Comparison", subtitle="Compare model performance side-by-side"), unsafe_allow_html=True)
    
    with st.expander("🔬 Compare Models Head-to-Head", expanded=False):
        compare_cols = st.columns([2, 2, 2, 2])
        with compare_cols[0]:
            compare_ticker = st.text_input("Symbol", value=bt_ticker, key="compare_ticker")
        with compare_cols[1]:
            compare_period = st.selectbox("Period", ["1y", "2y", "5y"], index=1, key="compare_period")
        with compare_cols[2]:
            compare_horizon = st.selectbox("Horizon", [1, 3, 5], index=0, key="compare_horizon")
        with compare_cols[3]:
            st.markdown("<div style='height: 28px'></div>", unsafe_allow_html=True)  # Spacer
            run_compare_btn = st.button("⚡ COMPARE ALL", key="run_compare", use_container_width=True)
        
        if run_compare_btn:
            compare_ticker_clean = compare_ticker.strip().upper()
            models_to_compare = ["xgb", "rf", "ensemble"]
            comparison_results = []
            progress_compare = st.progress(0)
            
            for i, m in enumerate(models_to_compare):
                result_data, api_error = safe_api_call(
                    _cached_track_predictions,
                    compare_ticker_clean,
                    period=compare_period,
                    model_type=m,
                    horizon=compare_horizon,
                    error_prefix=f"Backtest failed for {m}"
                )
                
                if result_data and not api_error:
                    results_test, accuracy = result_data
                    if results_test is not None and not results_test.empty:
                        baseline = results_test["actual_return"].dropna()
                        strat = results_test.copy()
                        strat["position"] = np.where(strat["predicted_return"] > signal_threshold_pct, 1.0, 0.0)
                        strat["strategy_ret"] = strat["actual_return"] * strat["position"]
                        
                        sharpe_baseline = compute_sharpe(baseline)
                        sharpe_strategy = compute_sharpe(strat["strategy_ret"])
                        total_ret = (1 + strat["strategy_ret"].dropna()).prod() - 1
                        dd_info = compute_drawdown(strat["strategy_ret"])
                        max_dd = dd_info.get("max_dd", 0) if dd_info else 0
                        
                        comparison_results.append({
                            "Model": {"xgb": "XGBoost", "rf": "Random Forest", "ensemble": "Ensemble"}[m],
                            "Sharpe": f"{sharpe_strategy:.2f}" if sharpe_strategy else "—",
                            "Accuracy": f"{accuracy * 100:.0f}%" if accuracy else "—",
                            "Total Return": f"{total_ret * 100:+.1f}%",
                            "Max Drawdown": f"{max_dd * 100:.1f}%" if max_dd else "—",
                            "vs B&H": f"{(sharpe_strategy - sharpe_baseline):+.2f}" if sharpe_strategy and sharpe_baseline else "—"
                        })
                
                progress_compare.progress((i + 1) / len(models_to_compare))
            
            progress_compare.empty()
            
            if comparison_results:
                compare_df = pd.DataFrame(comparison_results)
                st.markdown(render_styled_table(compare_df), unsafe_allow_html=True)
                
                # Highlight best model
                try:
                    sharpe_vals = [float(r["Sharpe"]) for r in comparison_results if r["Sharpe"] != "—"]
                    if sharpe_vals:
                        best_idx = sharpe_vals.index(max(sharpe_vals))
                        best_model = comparison_results[best_idx]["Model"]
                        show_toast(f"🏆 Best Model: {best_model} (Sharpe {comparison_results[best_idx]['Sharpe']})", "success")
                except:
                    pass
            else:
                show_alert("No comparison results available. Check ticker symbol and try again.", "warning")

# ============================================================================
# TAB: Portfolio Walk-Forward Analysis
# ============================================================================
elif active_tab == "portfolio":
    st.markdown(section_header("Portfolio Walk-Forward Analysis", subtitle="Multi-asset strategy performance"), unsafe_allow_html=True)
    ...
    if run_wf:
        ...
        if ticker_errors and not valid_tickers:
            ...
            show_alert("⚠️ No valid tickers provided. Please enter ticker symbols separated by commas.", "error")
        else:
            ...
            if not is_valid:
                ...
                try:
                    ...
                    if all_folds:
                        ...
                        show_toast(f"✅ Completed: {len(results_df)} folds across {success_count} assets", "success")
                    else:
                        show_alert("❌ No results generated. All tickers failed or had insufficient data.", "error")
    ...
    if "port_results" in st.session_state and st.session_state.port_results is not None:
        ...
        if not results_df.empty:
            ...
            st.markdown(section_header("Performance Summary"), unsafe_allow_html=True)
            ...
            with result_cols[0]:
                st.markdown(section_header("Fold Results"), unsafe_allow_html=True)
            ...
            with result_cols[1]:
                st.markdown(section_header("Sharpe Distribution"), unsafe_allow_html=True)
        ...
        st.markdown(
            render_empty_state(
                ...
                description="Configure your portfolio universe above and click RUN ANALYSIS to evaluate model robustness across multiple assets and time windows.",
                icon="📊"
            ),
            unsafe_allow_html=True
        )
...
# ============================================================================
    ...
    if not HAS_MONITORING:
        show_alert("⚠️ Performance monitoring module not available. Install with: `from src.monitoring import PerformanceMonitor`", "warning")
    else:
        ...
        if alerts:
            for alert in alerts:
                if alert.level == AlertLevel.CRITICAL:
                    show_alert(f"🚨 **{alert.category.upper()}**: {alert.message}", "error")
                elif alert.level == AlertLevel.WARNING:
                    show_alert(f"⚠️ **{alert.category.upper()}**: {alert.message}", "warning")
                else:
                    show_alert(f"ℹ️ **{alert.category.upper()}**: {alert.message}", "info")
        else:
            show_toast("✅ No active alerts - all metrics within normal ranges", "success")
        ...
        if ticker_breakdown:
            ...
            # Use styled_table for modern look
            st.markdown(styled_table(ticker_df), unsafe_allow_html=True)
        else:
            show_alert("No trades recorded yet. Trades will appear here after your first closed position.", msg_type="info")
        ...
        if monitor.trades:
            ...
            st.markdown(styled_table(
                trades_df,
                highlight_rules={
                    "P&L": lambda v: "positive" if "+" in str(v) and "-" not in str(v) else ("negative" if "-" in str(v) else None),
                }
            ), unsafe_allow_html=True)
        else:
            show_alert("No trades recorded yet.", "info")
        ...
        if summaries_path.exists():
            ...
            if summary_files:
                ...
                show_alert("No daily summaries available yet.", "info")
        else:
            show_alert("No daily summaries available yet. Summaries are generated at the end of each trading session.", "info")
        ...
        try:
            ...
            with cb_cols[0]:
                if cb_status["is_tripped"]:
                    show_alert(f"🛑 **TRIPPED**: {cb_status['trip_reason']}", "error")
                else:
                    show_toast("✅ **ACTIVE** - Trading enabled", "success")
            with cb_cols[1]:
            ...
            with st.expander("⚙️ Circuit Breaker Limits"):
                ...
                if st.button("⚠️ Force Reset Circuit Breaker", key="force_reset_cb"):
                    cb.force_reset()
                    show_toast("Circuit breaker reset!", "success")
                    st.rerun()
        except Exception as e:
            show_alert(f"Circuit breaker status unavailable: {e}", "warning")
...
# ============================================================================
# FLOATING ACTION BUTTON & KEYBOARD HINTS
# ============================================================================

# Render floating action button (only on summary and dashboard tabs)
if st.session_state.get("active_tab", "summary") in ["summary", "dashboard"]:
    st.markdown(render_floating_action_button(), unsafe_allow_html=True)

# Render keyboard shortcuts hint
st.markdown(render_keyboard_hints(), unsafe_allow_html=True)

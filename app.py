...
def run_app():
    ...
    if st.sidebar.button("Run Screener + Model"):
        ...
        for i, tk in enumerate(flagged):
            ...
            try:
                # ⭐ KEY CHANGE: Pass prediction_horizon to model
                out = predict_next_for_ticker(
                    tk,
                    period="5y",
                    model_type=model_type,
                    horizon=prediction_horizon,  # ← Using selected horizon
                ...
                if atm_iv is not None and last_close is not None:
                    try:
                        mc_res = option_mc_ev(
                            ...
                            sigma=float(atm_iv),
                            days=int(prediction_horizon),  # ← MC uses selected horizon
                            premium=1.0,
                            strike=float(last_close),
                            n_paths=5000,
                            is_call=True,
    ...
    if st.session_state.pred_df is not None:
        ...
        for _, row in pred_df.iterrows():
            strategy, sentiment = suggest_options_strategy(
                ...
                row.get("atm_iv"),
                horizon=display_horizon,  # ← Pass horizon to strategy suggestions
            )
            ...
            with st.spinner(f"Testing {test_ticker} {display_horizon_label} predictions..."):
                try:
                    results_test, accuracy = track_predictions(
                        ...
                        period="5y",
                        model_type=model_type,
                        horizon=display_horizon,  # ← Test uses selected horizon
                    )
            ...
            with st.spinner(f"Running OLS feature significance for {fs_ticker} ({display_horizon_label})..."):
                try:
                    ols_model, sig_df = analyze_feature_significance(
                        ticker=fs_ticker,
                        period="5y",
                        horizon=display_horizon,  # ← Feature analysis uses selected horizon
                        use_vol_scaled_target=False,
                    )
        ...
        multi_rows = []
        for h in [1, 2, 3, 4, 5]:  # ⭐ Expanded to 5 days
            try:
